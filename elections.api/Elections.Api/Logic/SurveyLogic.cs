...
{
    private readonly IMongoCollection<SurveyDB> _surveyCollection = database.GetCollection<SurveyDB>("surveys");
    ...
    public async Task<ApiResponse> SaveSurvey(SurveyReq req, string ip)
    {
        try
        {
            ...
            var existing = await _surveyCollection.Find(filter).FirstOrDefaultAsync();
            ...
            await _surveyCollection.UpdateOneAsync(filter, update, new UpdateOptions { IsUpsert = true });
    ...
    {
        return await _surveyCollection.Find(_ => true)
    ...
    {
        return await _surveyCollection.Find(x => x.Id == id)
            .Project(x => new SurveyResponse
            {
                Id = x.Id.ToString(),
                UserId = x.UserId,
    ...
    public async Task<List<PartyAggregate>> GetAggregatedResults()
    {
        ...
        var results = await _surveyCollection.Aggregate<PartyAggregate>(pipeline).ToListAsync();
        return results;
    }
    ...
    public async Task<ApiResponse> ToggleReaction(ObjectId surveyId, Guid userId, bool isLike)
    {
        try
        {
            var survey = await _surveyCollection.Find(x => x.Id == surveyId)
            ...
            await _surveyCollection.UpdateOneAsync(filter, update);
...
public class PartyAggregate
{
    ...
    public double AverageMandates { get; set; }
}
